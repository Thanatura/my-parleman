...
dossiers as (
    select
        ...
        d.titre_dossier,
        d.legislature,
        coalesce(da.est_adopte, false) as est_adopte
    from {{ source('raw_parleman', 'dossiers_parlementaires') }} d
    left join {{ ref('stg_dossier_actes') }} da on d.uid = da.dossier_uid
),
