...
groupes as (
  select organe_uid as groupe_uid,
...
groupe_members as (
  ...
  from groupes g
    left join depute_mandats dm on dm.organe_uid = g.groupe_uid
  group by g.groupe_uid,
    g.groupe_libelle,
    g.gp_position_politique,
    g.gp_couleur,
    g.groupe_legislature
...
interventions_avec_groupe as (
  ...
  from interventions i
    join depute_mandats dm on i.depute_uid = dm.depute_uid
    and (
      i.date_seance is null
      or (
        (
          dm.date_debut is null
          or i.date_seance >= dm.date_debut
        )
        and (
          dm.date_fin is null
          or i.date_seance <= dm.date_fin
        )
      )
    )
    join groupes g on dm.organe_uid = g.groupe_uid qualify row_number() over (
      partition by i.intervention_id
      order by coalesce(dm.date_fin, date('9999-12-31')) desc,
...
from groupe_members gm
  left join groupe_interventions gi on gm.groupe_uid = gi.groupe_uid