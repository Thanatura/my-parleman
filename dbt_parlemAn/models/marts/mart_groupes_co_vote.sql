with scrutins_groupes_enriched as (
  select groupe_uid,
    scrutin_uid,
    position_majoritaire
  from {{ ref('stg_scrutin_groupes_enriched') }}
)
select count(1) as nb_co_votes,
  s1.groupe_uid as groupe_uid_1,
  s2.groupe_uid as groupe_uid_2
from scrutins_groupes_enriched as s1
  join scrutins_groupes_enriched as s2 on s1.groupe_uid < s2.groupe_uid
